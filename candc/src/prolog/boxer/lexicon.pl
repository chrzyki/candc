...
%
semlex(Cat,_Sym,Index,Att-[sem:'NIL'|Att],Sem):- 
   Cat = (np\np)/n, !,
...
semlex(Cat,_Sym,Index,Att-[sem:'REL'|Att],Sem):-
   Cat = (np/np)/n, !,
...
% double prepositions: 
%
semlex(Cat,Sym,Index,Att-[sem:'ROL'|Att],Sem):-
   Cat = (np\np)/pp, !,
   att(Att,sense,Sense),
...
semlex(Cat,Sym,Index,Att-[sem:'RLI'|Att],Sem):-
   member(Cat,[(np\np)/np,(np\np)\np,(np/np)/np]),
   att(Att,scope,inv), !,
...
% Passive Clause
%
semlex(Cat,by,Index,Att1-[sem:'REL'|Att2],Sem):-
   Cat = ((s:pss\np)\(s:pss\np))/np, 
...
semlex(Cat,_Sym,Index,Att1-[sem:'REL'|Att2],Sem):-
   member(Cat,[(((s:X\np)\(s:X\np))/((s:X\np)\(s:X\np)))/n,
               (((s:X\np)\(s:X\np))\((s:X\np)\(s:X\np)))/n,
               (((s:X\np)\(s:X\np))\((s:X\np)\(s:X\np)))\n,
               (((s:X\np)\(s:X\np))\((s:X\np)\(s:X\np)))/pp]), !,
...
%
semlex(Cat,Sym,Index,Att1-[sem:'REL'|Att2],Sem):-
   member(Cat,[((s:X\np)\(s:X\np))/(s:to\np), 
               ((s:X\np)/(s:X\np))/(s:to\np)]), !,
...
%
semlex(Cat,Sym,Index,Att1-[sem:'QUE'|Att2],Sem):-
   Sym = who,
   member(Cat,[
               ((s:X\np)\(s:X\np))/(s:dcl\np), % which
               ((s:X\np)\(s:X\np))/(s:dcl/np), % who
...
semlex(Cat,Sym,Index,Att1-[sem:'EXN'|Att2],Sem):-
   att(Att1,pos,'VBN'),
   member(Cat,[(s:X/s:X)/pp,(s:X\s:X)/pp]), !, 
   plosing(CC),
   roles(Sym,(s:dcl\np)/s:_,[Role,_],Att1-Att2),
...
semlex((s:wq/(s:q/pp))/(s:wq/(s:q/np)),Sym,Index,Att-[sem:'REL'|Att],Sem):-
...
semlex(Cat,Sym,_,Att1-[sem:'PRO'|Att2],Sem):-
   Cat = (np\np)/((s:to\np)/np), !,
   roles(Sym,(s:dcl\np)/s:_,[Role,_],Att1-Att2),
...
semlex(Cat,_Sym,_,Att-[sem:'PRO'|Att],Sem):-
   member(Cat,[(np\np)/(s:_\np),(np\np)/(s:_/np)]),
   option('--elimeq',true), !,
   closing(CC),
...
semlex(Cat,_Sym,Index,Att-[sem:'PRO'|Att],Sem):-
   member(Cat,[(np\np)/(s:_\np), (np\np)/(s:_/np)]),
   option('--elimeq',false), !,
   closing(CC),
   Sem = lam(VP,lam(Q,lam(P,app(Q,lam(X,merge(app(app(VP,
...
semlex(Cat,Sym,Index,Att-[sem:'PRO'|Att],Sem):-
   member(Cat,[((np\np)\(np\np))/s:dcl]), !, 
...
semlex(Cat,Sym,Index,Att-[sem:'ITJ'|Att],Sem):-
   category(s,Cat,_), !,
   att(Att,sense,Sense),
